...
const Photo = ({ size, account, badges }) => {
    ...
    return (
        <div className={`h-${size} w-${size}`}>
            <div className={classNames(`inline-flex items-center justify-center h-${size} w-${size} rounded-full bg-gray-500`, (badges === true && isAlumni) && `ring ring-${ring_size} ring-violet-600 ring-offset-2`)}>
